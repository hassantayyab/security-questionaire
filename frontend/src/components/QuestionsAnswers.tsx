...
import { Questionnaire, Question, UploadResponse, GenerateAnswersResponse } from "@/types";
  ...
  const saveAnswer = async (questionId: string) => {
    try {
      ...
      setQuestions(prev => 
        prev.map(q => 
          q.id === questionId 
            ? { ...q, answer: editingAnswer, status: "unapproved" as const }
            : q
        )
      );
  ...
  const toggleApproval = async (questionId: string, currentStatus: string) => {
    try {
      ...
      if (newStatus === "approved") {
        ...
        await api.updateAnswer(questionId, questions.find(q => q.id === questionId)?.answer || "", "unapproved");
      }
      ...
      setQuestions(prev => 
        prev.map(q => 
          q.id === questionId 
            ? { ...q, status: newStatus as "approved" | "unapproved" }
            : q
        )
      );
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (response.success && response.approved_questions) {
        ...
        ].map(row => row.map((cell: any) => `"${cell.replace(/"/g, '""')}"`).join(",")).join("\n");
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload Excel files containing your security questionnaires. Questions will be parsed automatically.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <FileUpload
